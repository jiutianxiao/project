...
function bindData() {
    function callback(jsonData) {
        ...
        for (var i = 0, len = data.length; i < len; i++) {
            ...
            str += '<a href="add.html?num=' + curData["num"] + '">详情</a>'
            str += '</li>';
        }
...
boxList.onclick = function (ev) {
    ...
    if (tarTag === "A" && tar.innerHTML === "删除") {
        var cusNum = tar.getAttribute("cusNum");
        console.log(tar)
        console.log(tar.getAttribute("cusNum"))
        var flag = window.confirm("确定要删除编号为[" + cusNum + "]的信息么？");
        if (!flag) {
            return;
        }
        ajax({
            url: "/remove?num=" + cusNum + "&_=" + Math.random(),
            type: "get",
